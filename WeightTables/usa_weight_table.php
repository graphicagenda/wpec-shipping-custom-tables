<?php
/*
Plugin Name: WP E-Commerce - USA Weight Table
...
class usa_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("usa_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_usa'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_usa') {
			update_option('usa_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		if ($layers != '' && $country == 'US') {
			...
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("Ship to USA"=>$shipping);
				}
			}
		...
		return array("USA"=>array_shift($layers));
		}
		else {
		return array();
		}
...
function usa_shipping_add($wpsc_shipping_modules) {

	global $usa_shipping;
	
	$usa_shipping = new usa_shipping();
	$wpsc_shipping_modules[$usa_shipping->getInternalName()] = $usa_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'usa_shipping_add');
?>