<?php
/*
Plugin Name: WP E-Commerce - USA Continental Weight Table
...
class usaC_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("usaC_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_usaC'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_usaC') {
			update_option('usaC_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		if ($layers != '' && $country == 'US') {
			...
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("Free Shipping in the Continental US"=>$shipping);
				}
			}
		...
		return array("USAC"=>array_shift($layers));
		}else{
		}}
		else {
		return array();
...
function usaC_shipping_add($wpsc_shipping_modules) {

	global $usaC_shipping;
	
	$usaC_shipping = new usaC_shipping();
	$wpsc_shipping_modules[$usaC_shipping->getInternalName()] = $usaC_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'usaC_shipping_add');
?>