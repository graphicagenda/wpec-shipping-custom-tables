<?php
/*
Plugin Name: WP E-Commerce - Canada Weight Table
Plugin URI: http://getshopped.org/resources/docs/get-involved/writing-a-new-shipping-module/
Description: Custom Shipping Module For Shipping to Canada
Version: 0.8
...
class can_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("can_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_can'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_can') {
			update_option('can_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		$layers = get_option('can_shipping_options');
		if ($layers != '' && $country == 'CA') {
			krsort($layers);
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("Ship to Canada"=>$shipping);
				}
			}
		...
		return array("Canada"=>array_shift($layers));
		}
		else {
		return array();
		}
...
function can_shipping_add($wpsc_shipping_modules) {

	global $can_shipping;
	
	$can_shipping = new can_shipping();
	$wpsc_shipping_modules[$can_shipping->getInternalName()] = $can_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'can_shipping_add');
?>