<?php
/*
Plugin Name: WP E-Commerce - USA Inter-Continental Weight Table
...
class usaI_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("usaI_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_usaI'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_usaI') {
			update_option('usaI_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		if ($layers != '' && $country == 'US') {
			if($region == 15 || $region == 24 ){
			krsort($layers);
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("$25 Inter Continental Shipping"=>$shipping);
				}
			}
		...
		return array("USAI"=>array_shift($layers));
		}else{
		}}
		else {
		return array();
...
function usaI_shipping_add($wpsc_shipping_modules) {

	global $usaI_shipping;
	
	$usaI_shipping = new usaI_shipping();
	$wpsc_shipping_modules[$usaI_shipping->getInternalName()] = $usaI_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'usaI_shipping_add');
?>