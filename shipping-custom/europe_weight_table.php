<?php
/*
Plugin Name: WP E-Commerce - International Weight Table
Plugin URI: http://getshopped.org/resources/docs/get-involved/writing-a-new-shipping-module/
Description: Custom Shipping Module For International Shipping
Version: 0.8
...
class intl_shipping {
	...
	function intl_shipping () {

		// An internal reference to the method - must be unique!
		$this->internal_name = "intl_shipping";
	...
	//	$output ="<table>";
		...
		$layers = get_option("intl_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_intl'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_intl') {
			update_option('intl_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		$layers = get_option('intl_shipping_options');
		if ($layers != '' && $country != 'CA' && $country != 'AU' && $country != 'US'  && $country != 'MX') {
			krsort($layers);
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("International Shipping"=>$shipping);
				}
			}
		...
		return array("International"=>array_shift($layers));
		}
		else {
		return array();
		}
...
function intl_shipping_add($wpsc_shipping_modules) {

	global $intl_shipping;
	
	$intl_shipping = new intl_shipping();
	$wpsc_shipping_modules[$intl_shipping->getInternalName()] = $intl_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'intl_shipping_add');
?>