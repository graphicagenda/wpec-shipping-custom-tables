<?php
/*
Plugin Name: WP E-Commerce - Mexico Weight Table
Plugin URI: http://getshopped.org/resources/docs/get-involved/writing-a-new-shipping-module/
Description: Custom Shipping Module For Shipping to Mexico
Version: 0.9
...
class mex_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("mex_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_mex'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_mex') {
			update_option('mex_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		$layers = get_option('mex_shipping_options');
		if ($layers != '' && $country == 'MX') {
			krsort($layers);
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("Ship to Mexico"=>$shipping);
				}
			}
		...
		return array("Mexico"=>array_shift($layers));
		}
		else {
		return array();
		}
...
function mex_shipping_add($wpsc_shipping_modules) {

	global $mex_shipping;
	
	$mex_shipping = new mex_shipping();
	$wpsc_shipping_modules[$mex_shipping->getInternalName()] = $mex_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'mex_shipping_add');
?>