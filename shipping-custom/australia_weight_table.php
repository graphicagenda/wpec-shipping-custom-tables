<?php
/*
Plugin Name: WP E-Commerce - Australia Weight Table
Plugin URI: http://getshopped.org/resources/docs/get-involved/writing-a-new-shipping-module/
Description: Custom Shipping Module For Shipping to Australia
Version: 0.8
...
Original Plugin URI: http://getshopped.org/resources/docs/get-involved/writing-a-new-shipping-module/
 ...
 Version: 0.8 - getQuote Australia array
 Version: 0.5 - General fixes 03/31/10 
...
class aus_shipping {
	...
	//	$output ="<table>";
		...
		$layers = get_option("aus_shipping_options");
		if ($layers != '') {
		...
		$output.="<input type='hidden' name='checkpage' value='weight_aus'>";
	...
	function submit_form() {
		...
		if ($_POST['checkpage'] == 'weight_aus') {
			update_option('aus_shipping_options',$new_layer);
		}
		return true;
	...
	function getQuote() {
		...
		$layers = get_option('aus_shipping_options');
		if ($layers != '' && $country == 'AU') {
			krsort($layers);
			foreach ($layers as $key => $shipping) {
				if ($weight >= (float)$key) {
					return array("Ship to Australia"=>$shipping);
				}
			}
		...
		return array("Australia"=>array_shift($layers));
		}
		else {
		return array();
		}
...
function aus_shipping_add($wpsc_shipping_modules) {

	global $aus_shipping;
	
	$aus_shipping = new aus_shipping();
	$wpsc_shipping_modules[$aus_shipping->getInternalName()] = $aus_shipping;

	return $wpsc_shipping_modules;
}

add_filter('wpsc_shipping_modules', 'aus_shipping_add');
?>